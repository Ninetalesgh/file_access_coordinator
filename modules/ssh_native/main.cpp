...
int upload_file(const char* localPath, char const* remotePath)
{
  ...
  if (sftp == NULL)
  {
    log_error("- Error creating SFTP session: ", ssh_get_error(session));
    return SSH_ERROR;
  }

  if (sftp_init(sftp) != SSH_OK)
  {
    log_error("- Error initializing SCP: ", ssh_get_error(session));
    sftp_free(sftp);
    return SSH_ERROR;
  }
  ...
  if (remoteFile == NULL)
  {
    log_error("- Error opening remote file: ", ssh_get_error(session));
  }
  ...
  while ((bytesRead = fread(buffer,1, sizeof(buffer), localFile)) > 0)
  {
    ...
    {
      log_error("- Error while writing remote file: ", ssh_get_error(session));
      break;
    }
  }
...
int download_file(char const* localPath, char const* remotePath)
{
  ...
  if (bytesRead < 0)
  {
    log_error("- Error while reading remote file: ", ssh_get_error(session));
  }
...
int init(char const* user, char const* sshUser, char const* sshHost, char const* sshPassword)
{
  log_info("\n==============================================");
  log_info("----------------------------------------------");
  ...
  if (rc != SSH_OK)
  {
    log_error("Connection failed: ", ssh_get_error(session));
    ssh_free(session);
    return SSH_ERROR;
  }
  ...
  {
    log_error("Authentication failed: ", ssh_get_error(session));
    ssh_free(session);
    return SSH_ERROR;
  }
  ...
  request_exec(stringFormatBuffer, responseBuffer, sizeof(responseBuffer), false);
  string_copy(ipAddress, responseBuffer, sizeof(ipAddress));
  string_replace_char(ipAddress,'\n','\0');

  log_info("- Connected to '", sshUser, "@", sshHost, "'");
  log_info("- User: ", user);
  log_info("- Local IP: ", ipAddress);
...
int main(int argc, char** argv)
{
  ...
  if (SSH_OK != init(user, sshUsername, sshHostname, password))
  {
    return 1;
  }

  string_format(fullRemotePath, sizeof(fullRemotePath), remoteDir, "/", filename);

  if (SSH_OK == reserve_remote_file_for_local_user(remoteDir, filename, user, ipAddress))
  {
    download_file(filename, fullRemotePath);
  
    if (SSH_OK == upload_file(filename, fullRemotePath))
    {
    }
  }

  release_remote_file_from_local_user(remoteDir, filename, user, ipAddress);

  shutdown();


  //request_exec_format("cd ", remote_dir, " && stat -c '%a' ", filename  );


//  request_exec("echo 'hello2'");
//  request_exec("cd database_service_dir && touch dummie && chmod -w dummie && stat -c '%a' dummie");
//
//  //upload_file("testfile.txt", "database_service_dir/testfile.txt");
//  download_file("testfile.txt", "database_service_dir/testfile.txt");
//  request_exec("stat -c '%a' database_service_dir/testfile.txt");
